...
def projection(network, optimizer, tl, MNIST_tran_ini, stat, saving, mb, itr):
    ...
    #reg =  2 ** itr 
    #reg =  2 ** ( itr - 1 )  this result is good
    #reg = 0.5 * itr
    #if itr > 2:
    #    reg = 10 * ( itr - 2 ) 
    #else:
    ...
    stat['cp'][( itr + 1 )] = ot.optim.cg(ps, pt, cost, reg, f, df, G0 = None, verbose=True)
...
def minibatch_data( stat, source_train, source_val, target_train, target_val ):
    ...
    vr = stat['vl_ratio']
    dr = stat['dl_ratio']
    b1, b2 = max(int(len( source_train) / ratio ), 300), max(int(len( target_train) /ratio), 300)
   ...
   #     target_data, [b2, len(target_data)- b2 ]) 
    ...
    stat['source'] = torch.utils.data.Subset(source_train, s_list)
    stat['s_val'] = torch.utils.data.Subset(source_val, s_list)
    
    stat['target'] = torch.utils.data.Subset(target_train, t_list)
    stat['t_val'] = torch.utils.data.Subset(target_val, t_list)
    
    # In[6]:
    # define train loader and validation loader
    stat['svl'] = DataLoader( stat['s_val'], batch_size= int( b1 / vr ), 
                                 shuffle=False, drop_last = False)
    stat['tvl'] = DataLoader( stat['t_val'], batch_size= int( b2 / vr ), 
                                 shuffle=False, drop_last = False)
    ...
    a, b = stat['source'][3]
    a2, b2 = stat['s_val'][3]
    images, labels = stat['target'][3]
    images2, labels2 = stat['t_val'][3]
    # show images
    imshow(thv.utils.make_grid(a))
    imshow(thv.utils.make_grid(a2))
    imshow(thv.utils.make_grid(images))
    imshow(thv.utils.make_grid(images2))
...
def copy_traject( stat, mb ):
    for period in range( int(stat['num_period']*5)  ):
        interval_load = int(stat['interval']/5)
